...
- The <span style="color:blue"> mean number </span> of steps taken per day is: `r meansteps`

- The <span style="color:red"> median number </span> of steps taken per day is: `r mediansteps`
...
average steps within the day is interval `r maxinterval` with `r maxstepsasint` steps.
...
In the original activity data set there were `r missingcount` missing values in 
the steps column. 
...
increased by `r meanvar` and the median by `r medianvar`.
...
per day is: `r newmeansteps`. Overall this does not seem to negatively impact 
...
With the max number of steps being `r weekdaymax` on weekdays and `r weekendmax`
...
taken is higher on weekends (`r weekdayavg` / weekday versus `r weekendavg`
